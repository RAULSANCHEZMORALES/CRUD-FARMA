...
import LoadingInsumos from '../components/LoadingInsumos';
...
function RegistroInsumos() {
  ...
  useEffect(() => {
    setTimeout(() => {
     ...
     // Variables a utilizar para registro de insumos
  ...
  useEffect(() => {
   ...
   //Función de añadir nuevos insumos
  const add = (val) =>{
     // Validar campos aquí

  if (!nombre_insumo || !unidad_medicion || !fabricante || !distribuidora || !lote || !tabla_insumo || !fecha_ingreso || !fecha_caducidad || !num_piezas || !cantidad_unidad || !total || !codigo_insumo || !codigo_categoria || !codigo_estatus || !observaciones) {
    Swal.fire({
      title: 'Campos incompletos',
      text: 'Por favor, completa todos los campos antes de registrar el insumo.',
      icon: 'warning',
    ...
    console.log(unidadTotalEnGramos)
      ...
      'Material Oficina': '03',
      'Equipo E Instrumentos': '04',
      'Insumo De Laboratorio': '05',
    ...
    Swal.fire({
      ...
      html: "<i>Seguro que quieres registrar al insumo <strong>"+nombre_insumo+"</strong> ? </i>",
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        Axios.post("http://localhost:3001/insumos/registrar_Insumo",{
          id_insumo:id_insumo,
          ...
          Swal.fire({
            ...
            html: "<i>Se agrego el insumo <strong>"+nombre_insumo+"</strong> correctamente </i>",
            icon: 'success',
            timer:5000
        })
    });
  ...
  const actualizar = () =>{
    ...
    const codigoFinalValue = codigoCategoria + lote + codigo_insumo;
    setCodigo_Final(codigoFinalValue);
    Axios.put("http://localhost:3001/insumos/actualizar_Insumo",{
     id_insumo:id_insumo,
     nombre_insumo:nombre_insumo,
     unidad_medicion:unidad_medicion,
     fabricante:fabricante,
     ...
     Swal.fire({
      ...
      html: "<i>Se edito la información del insumo <strong>"+nombre_insumo+"</strong> correctamente </i>",
      icon: 'success',
      timer:5000
    })
    });
  ...
  const eliminar = (val) =>{
    Swal.fire({
      title: 'Espera...',
      html: "<i>Seguro que quieres eliminar el insumo <strong>"+val.nombre_insumo+"</strong> ? </i>",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        Axios.delete(`http://localhost:3001/insumos/eliminar_Insumo/${val.id_insumo}`,
        ).then(()=>{
         getInsumos();
         limpiarCampos();
         Swal.fire(
          'Insumo Eliminado',
          val.nombre_insumo+' se ha borrado correctamente!',
          'success'
        );
        }).catch(function(err){
          Swal.fire({
   ...
   const editarInsumos = (val)=>{
       ...
       setNombre_Insumo(val.nombre_insumo);
...
const handleChange = (e) => {
  ...
  if (terminoBusqueda) {
    const resultadosBusqueda = insumosList.filter((elemento) => {
      const nombre = elemento.nombre_insumo.toString().toLowerCase();
      ...
      const codigoCategoria = elemento.codigo_categoria.toString().toLowerCase();
      ...
      return (
        ...
        codigoCategoria.includes(terminoBusqueda.toLowerCase())
      );
    });
  ...
  <div className="row">
    <div className="col-md-6">
      <div className="card text-center">
        ...
        <strong>REGISTRO DE NUEVOS INSUMOS</strong>
        </div>
        <div className="card-body">
          <div className="input-group mb-3">
            ...
            <input
              type="text"
              onChange={(event) => {
                setNombre_Insumo(event.target.value);
              }}
            ...
            <div className="form-check">
              ...
              />
              <label className="form-check-label" htmlFor="unidad-medicion">
                Gramos
              </label>
            </div>
            ...
            <div className="form-check">
              ...
              />
              <label className="form-check-label" htmlFor="unidad-medicion-2">
                Kilogramos
              </label>
            </div>
            ...
            <div className="form-check">
              ...
              />
              <label className="form-check-label" htmlFor="unidad-medicion-3">
                Mililitros
              </label>
              </div>
              ...
              />
              <label className="form-check-label" htmlFor="unidad-medicion-4">
                Piezas
              </label>
              </div>
            </div>
        ...
        <div className="input-group-text">
          <input
            ...
            onChange={(event) => {
              ...
              setFabricante(isChecked ? "Sin Datos" : "Sin Datos");
            }}
        ...
        <div className="input-group-text">
          <input
            ...
            onChange={(event) => {
              ...
              setDistribuidora(isChecked ? "Sin Datos" : "Sin Datos");
            }}
            aria-label="Checkbox for no expiry date"
          />
          <span style={{ marginLeft: '5px' }}>Sin Datos</span>
  ...
  <div className="d-flex align-items-center">
    <div className="form-check">
      <input
        ...
        id="tabla-insumo"
        value={tabla_insumo}
        checked={tabla_insumo === "insumos"}
        onChange={(event) => {
          setTabla_Insumo(event.target.checked ? "insumos" : "");
        }}
      />
      <label
        className="form-check-label"
        htmlFor="tabla-insumo"
        style={{ backgroundColor: tabla_insumo === "insumos" ? "rgba(0, 0, 255, 0.5)" : "" }}
  ...
  <div className="check-container">
    <div className="form-check">
      ...
      />
      <label className="form-check-label" htmlFor="option1" title="Materia Prima: 01">
        Codigo 01
      </label>
    </div>
    <div className="form-check">
      ...
      />
      <label className="form-check-label" htmlFor="option2" title="Material Envase Empaque: 02">
        Codigo 02
      </label>
    </div>
    <div className="form-check">
      <input
        ...
        id="option3"
        value={codigo_categoria}
        checked={codigo_categoria === "Material Oficina"}
        onChange={(event) => {
          setCodigo_Categoria(event.target.checked ? "Material Oficina" : "");
        }}
      />
      <label className="form-check-label" htmlFor="option3" title="Material Oficina: 03">
        Codigo 03
      </label>
    </div>
    <div className="form-check">
      <input
        ...
        id="option4"
        value={codigo_categoria}
        checked={codigo_categoria === "Equipo E Instrumentos"}
        onChange={(event) => {
          setCodigo_Categoria(event.target.checked ? "Equipo E Instrumentos" : "");
        }}
      />
      <label className="form-check-label" htmlFor="option4" title="Equipo E Instrumentos: 04">
        Codigo 04
      </label>
    </div>
    <div className="form-check">
      <input
        ...
        id="option5"
        value={codigo_categoria}
        checked={codigo_categoria === "Insumo De Laboratorio"}
        onChange={(event) => {
          setCodigo_Categoria(event.target.checked ? "Insumo De Laboratorio" : "");
        }}
      />
      <label className="form-check-label" htmlFor="option5" title="Insumo De Laboratorio: 05">
        Codigo 05
      </label>
    </div>
  </div>
      ...
      <div className="form-check">
        ...
        />
        <label className="form-check-label" htmlFor="estatus-1" style={{ backgroundColor: codigo_estatus === "Cuarentena" ? "rgba(255, 255, 0, 0.5)" : "" }}>
          Cuarentena
        </label>
      </div>
      <div className="form-check">
        ...
        />
        <label className="form-check-label" htmlFor="estatus-2" style={{ backgroundColor: codigo_estatus === "Rechazado" ? "rgba(255, 0, 0, 0.5)" : "" }}>
          Rechazado
        </label>
      </div>
      <div className="form-check">
        ...
        />
        <label className="form-check-label" htmlFor="estatus-3" style={{ backgroundColor: codigo_estatus === "Aprobado" ? "rgba(0, 255, 0, 0.5)" : "" }}>
          Aprobado
        </label>
      </div>
    </div>
       ...
       :  <button className='btn btn-success' style={{ marginTop: '20px' }} onClick={add}>Registrar Nuevo Insumo</button>
       }
            ...
            <input
              ...
              value={busqueda}
              placeholder="Búsqueda por Nombre Insumo, Fabricante, Distribuidora, Lote, Codigo Final o Codigo Categoria"
          ...
          <thead>
            <tr>
              ...
              <th scope="col" style={{textAlign: "left", padding: "2px"}}>Nombre Insumo</th>
          ...
          <tbody>    
          {
        // Listas todos los componentes de insumos
            busqueda ?
            insumos_Buscar.map((val,key)=>{
                return <tr key={val.id_insumo}>
                            <th>{val.id_insumo}</th>
